...
version: V2.4.2 纯Python标准库版 + 场景自动识别 + 睡眠/运动双模式HRV分析 + 运动模式细分识别(个体化归一化+爬山识别+短样本门控) + HRV指标悬停解读 + HTML报告(Chart.js自包含) + 心内科综合分析
...
agent_created: true
---
# heartrate-sensor-parser 技能使用手册 V2.4.2
...
### 6.5 已知样本表现（用户标注对照，V2.4.2）
| 日志 | 用户标注 | 分类结果 | drift_pct | 说明 |
|------|----------|----------|-----------|------|
| 0712-pb-1 | 跑步 | 跑步 0.79（高置信） | 0.524 | 有 15min 热身，drift 87 bpm/h |
| 0712-pb-2 | 跑步 | 跑步 0.63（中置信） | 0.274 | 无热身直接高强度稳态 |
| 0711_ps-1 | 爬山(上山) | 爬山 0.89（高置信） | 1.053 | 极端上行速率触发 uphill_extreme 满档 |
| 0711_ps-2 | 爬山(上下山) | 爬山 0.49（中置信） | -0.139 | 负漂移+高双峰 0.72 触发 downhill_signal |
| 0712_ps-1 | 爬山(上山) | 爬山 0.83（高置信） | 0.800 | uphill_extreme 满档 |
| 0712_ps-2 | 爬山(上下山) | 爬山 0.71（高置信） | -0.234 | 负漂移+极高双峰 0.88 |
| 0701 | 骑行+跑步 | 混合 0.45（低置信 LC+IA） | 0.024 | 混合运动段，诚实降级 |
| 0709-2 | 骑行 | 混合 0.45（低置信 LC+IA） | -0.007 | 高间歇+零漂移，形态难分 |
| 0709-1 | 骑行 | 跑步 0.74（中置信） | 0.463 | ⚠️ **纯心率无解死结**：drift+climb+load 三项全部落在跑步区，需 GPS/踏频区分 |
| 0629 | — | 混合 0.45（低置信） | -0.074 | 特征区分度有限，诚实降级 |
...
10. **真实跑步被判成骑行（间歇跑）**：高强度间歇跑与间歇骑行的心率形态高度重叠（高间歇+高尖峰+零漂移），纯心率无法区分。V2.4.1 起对此形态置 `interval_ambiguous=True`，徽章降级「中置信(难分)」并显示蓝色提示，不再武断高置信判骑行。切割日志的后半段会截断漂移信号（drift≈0），可拼回完整段以恢复漂移特征。切勿再靠加 `climb` 加权强判——会误伤上山骑行样本。
11. 百分比/数值显示精度溢出（V2.2.4 已修复）：数值显示统一 `:.2f` 格式化。
