...
version: V2.4.1 纯Python标准库版 + 场景自动识别 + 睡眠/运动双模式HRV分析 + 运动模式细分识别(分段drift+间歇难分标记) + HRV指标悬停解读 + HTML报告(Chart.js自包含) + 心内科综合分析
...
| 0701-1 | 骑行（真实） | 骑行 0.57（中置信） | 前段，与标注吻合 |
| 0701-2 | 跑步（真实间歇） | 骑行 0.64 → **中置信(难分)** | 高间歇+零漂移，形态与骑行重叠，触发 `interval_ambiguous` 降级 |
| 0709-2 | 骑行 | 骑行 0.576（中置信） | 诚实输出，未过度自信 |
| 0709-1 | 骑行 | 混合（低置信） | 特征区分度有限，诚实降级（climb 回退后不再误判跑步） |
| 0629 | — | 混合 0.45（低置信） | 区分度有限，诚实降级 |

> 启发式分类器对骑行的召回受限于真实骑行摆动手腕导致 HR 波动；若需高可信判定，必须融合 GPS/踏频/加速度计等外部传感器。
>
> **死结案例（0701-2 跑步 vs 0709-2 骑行）**：两者形态几乎完全一致（高波动 var≈9-13、高间歇 inter≈31-33、高尖峰、零漂移），但用户标注不同——纯心率下无区分度。任何仅调 `drift`/`climb` 时域特征的尝试都无法在不误伤对方前提下把 0701-2 判成跑步。诚实处理即 `interval_ambiguous` 降级 + 提示融合传感器，而非硬判。0701-2 因是切割的后半段、起始已在高位，漂移信号被截断（drift≈0），若拼回完整 0701 段漂移特征会更完整。
